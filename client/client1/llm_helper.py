...
class LLMHelper:
    ...
    def generate_module_overview(
        ...
        body = file_content[:12000]
        if len(file_content) > 12000:
            body += f"\n\n… (file continues, {len(file_content) - 12000} more characters not shown)"
...
Be thorough and precise. Base everything strictly on the code above."""
        ...
        result = self._call(prompt, num_predict=3000, temperature=0.15, timeout=300)
...
Respond with ONLY a valid JSON object where each key is a symbol name and the value is a 3–5 sentence documentation paragraph. No markdown fences, no extra keys."""
        ...
        text = self._call(prompt, num_predict=4000, temperature=0.15, timeout=360)
